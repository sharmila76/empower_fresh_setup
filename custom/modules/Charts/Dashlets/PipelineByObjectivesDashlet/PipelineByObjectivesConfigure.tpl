...
<script type="text/javascript">
	function enableSearchDate() {
		var beginDateEle = document.getElementById("date_start");

		if (ele.value == "1") {
			beginDateEle.parent().parent().style = "display:inline";
		} else {
			beginDateEle.parent().parent().style = "display:none";
		}
	}
</script>
...
<tr>
    ...
    <td valign='top' class='dataField'>
    	<input type="text" value="{$dashlet_title}" size="30" name="dashlet_title"/>
    </td>
</tr>
<tr style="display:{SEARCH_DATE_DISPLAY}">
    <td valign='top' nowrap class='dataLabel'>{$LBL_DATE_START} <br><i>{$user_date_format}</i></td>
    <td valign='top' class='dataField'>
    	<input onblur="parseDate(this, '{$cal_dateformat}');" class="text" name="date_start" size='12' maxlength='10' id='date_start' value='{$date_start}'>
...
<tr style="display:{SEARCH_DATE_DISPLAY}">
    ...
    <td valign='top' class='dataField'>
    	<input onblur="parseDate(this, '{$cal_dateformat}');" class="text" name="date_end" size='12' maxlength='10' id='date_end' value='{$date_end}'>
    	<img src="{sugar_getimagepath file='jscalendar.gif'}" alt="{$LBL_ENTER_DATE}" id="date_end_trigger" align="absmiddle">
    </td>
</tr>
<tr>
    <td valign='top' class='dataLabel' nowrap>{$LBL_TYPE}</td>
    <td valign='top' class='dataField'>
    	<select name="obj_status_types[]" multiple size='3'>
    		{$selected_datax}
    	</select>
    </td>
</tr>
