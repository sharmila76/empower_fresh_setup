 ...
 * @return
 */
function displayObjectiveValueOptions() {
	var type = document.getElementById("objective_value_type").value;
		document.getElementById("objective_value").style.visibility = 'visible';
		document.getElementById("objective_value").style.display = 'inline';
		document.getElementById("objective_value_label").style.visibility = 'visible';
		document.getElementById("OBJ_ObjectivesobjectiveValuesTable0").style.visibility = 'visible';
}


/**
 * Objective Value Type Multiple Options Generation
 */
(function() {
	//Do not double define
	if (SUGAR.ObjectiveValuesWidget) return;
	
	var Dom = YAHOO.util.Dom;
	
	SUGAR.ObjectiveValuesWidget = function(module) {
		if (!SUGAR.ObjectiveValuesWidget.count[module]) SUGAR.ObjectiveValuesWidget.count[module] = 0;
		this.count = SUGAR.ObjectiveValuesWidget.count[module];
		SUGAR.ObjectiveValuesWidget.count[module]++;
		this.module = module;
		this.id = this.module + this.count;
		if (document.getElementById(module+'_objective_value_widget_id'))
		    document.getElementById(module+'_objective_value_widget_id').value = this.id;
		SUGAR.ObjectiveValuesWidget.instances[this.id] = this;
	}
	
	SUGAR.ObjectiveValuesWidget.instances = {};
	SUGAR.ObjectiveValuesWidget.count = {};
	
	SUGAR.ObjectiveValuesWidget.prototype = {
	    objectiveValueTemplate : '<tr id="objectiveValueRow">' +
	    '<td nowrap="NOWRAP"><input type="text" name="assigned_user_id{$index}" id="assigned_user_id0" size="30"/></td>' + 
		'<td><input type="text" name="objectiveValue{$index}" id="objectiveValue0" size="30"/></td>' + 
		...
		numberObjectiveValues : 0,
        enterPressed : false,
		tabPressed : false,
        objectiveValueView:"",
		objectiveValueIsRequired: false,
		tabIndex: -1,
		deletedIndex: 0,
		
		prefillObjectiveValues: function(tableId, o){
			for (i = 0; i < o.length; i++) {
				this.addObjectiveValue(tableId, o[i].id, o[i].assigned_user_id, o[i].objective_value);
			}
		},
		
		retrieveObjectiveValue: function (event) {
            var callbackFunction = function success(data) {
	            var vals = YAHOO.lang.JSON.parse(data.responseText);
	            var target = vals.target;
		        event = this.getEvent(event);
				
		        var index = /[a-z]*\d?objectiveValue(\d+)/i.exec(target)[1];
		        ...
		        if(savePressed || this.enterPressed) {
		           setTimeout("SUGAR.ObjectiveValuesWidget.instances." + this.id + ".forceSubmit()", 2100);
		        } else if(this.tabPressed) {
		           Dom.get(this.id + 'objectiveValuePrimaryFlag' + index).focus();
		        }
		    }
	    },
		...
		addObjectiveValue : function (tableId, db_id, assigned_user_id, objective_value) {
		    ...
		    var objectiveValueEle = document.createElement("input");

		    var listSize = document.getElementById("objectiveValueListSize");
		    listSize.setAttribute("value", this.numberObjectiveValues + 1);
		    ...
		    var objValIdEle = document.createElement("input");
		    objValIdEle.setAttribute("type", "hidden");
		    objValIdEle.setAttribute("name", "db_id" + this.numberObjectiveValues);
		    objValIdEle.setAttribute("id", "db_id" + this.numberObjectiveValues);
		    
		    if (db_id != '') {
		    	objValIdEle.setAttribute("value", db_id);
		    }
		    
		    // objective values
		    objectiveValueEle.setAttribute("type", "text");
		    objectiveValueEle.setAttribute("name", this.id + "objectiveValue" + this.numberObjectiveValues);
		    objectiveValueEle.setAttribute("id", this.id + "objectiveValue" + this.numberObjectiveValues);
		    objectiveValueEle.setAttribute("tabindex", this.tabIndex);
		    objectiveValueEle.setAttribute("size", "30");
		    objectiveValueEle.setAttribute("onKeyUp", "field_check(this.name)");
		    if (objective_value != '') {
		        objectiveValueEle.setAttribute("value", objective_value);
		    }
		    
		    // remove button
		    removeButton.setAttribute("id", this.id + "removeButton" + this.numberObjectiveValues);
			removeButton.setAttribute("class", "id-ff-remove");
		    removeButton.setAttribute("name", this.numberObjectiveValues);
		    ...
		    removeButton.onclick = function(){this.eaw.removeObjectiveValue(this.name);};
		
		    tr.setAttribute("id", this.id + "objectiveValueRow" + this.numberObjectiveValues);
		    
/////////////////////////////////////////////////////////////////////////////////////////////
		    td1.setAttribute("nowrap", "NOWRAP");
		    
		    var userList = Dom.get('user_list').cloneNode(true);
		    userList.style.display = 'inline';
		    userList.name = this.id + "assigned_user_id" + this.numberObjectiveValues;
		    userList.id = this.id + "assigned_user_id" + this.numberObjectiveValues;
		    if (assigned_user_id != '') {
		    	var options = [];
		    	options = userList.options;
		    	for (var i = 0; i < options.length; i++) {
		    		if (options[i].value == assigned_user_id) options[i].selected = "selected";
		    	}
		    }

		    td1.appendChild(userList);
		    td1.appendChild(objValIdEle);

/////////////////////////////////////////////////////////////////////////////////////////////

		    td2.appendChild(objectiveValueEle);

		    spanNode = document.createElement('span');
		    spanNode.innerHTML = '&nbsp;';
		    td3.appendChild(spanNode);
		    if (this.numberObjectiveValues != -1 || typeof (this.objectiveValueIsRequired) == "undefined" || !this.objectiveValueIsRequired)
	    	...
	    	addToValidate('EditView', this.id + "assigned_user_id" + this.numberObjectiveValues, 'duplicate', false, SUGAR.language.get('OBJ_Objectives', 'ERROR_DUPLICATE_USER'));
	    	addToValidate('EditView', this.id + "objectiveValue" + this.numberObjectiveValues, 'float', false, SUGAR.language.get('OBJ_Objectives', 'ERROR_NOT_A_NUMBER'));		    	

		    this.numberObjectiveValues++;
			this.addInProgress = false;
			
			//reload user can`t change assigned user name
			var assign_user = document.getElementById(this.id+ "assigned_user_id" + (this.numberObjectiveValues-1));
			var obj_value =  document.getElementById(this.id+ "objectiveValue" + (this.numberObjectiveValues-1));
			
			for(i=assign_user.options.length-1;i>=0;i--)
			{
				if(assign_user.options[i].value == assign_user.value)
					this_index = "selectedIndex="+i
			}
			if(obj_value.value != ''){
				assign_user.setAttribute("readonly",true ,0);
				assign_user.setAttribute("onchange",this_index,0);
			}
			
		}, //addObjectiveValue
		
		removeObjectiveValue : function(index) {
			var removedIndex = parseInt(index);

		    // Remove From Validation Array
		    removeFromValidate('EditView', this.id + "assigned_user_id" + (removedIndex + 1));
		    removeFromValidate('EditView', this.id + "objectiveValue" + (removedIndex + 1));

			var db_id = document.getElementById('db_id' + index).value;
		    var objValIdEle = document.createElement("input");
		    objValIdEle.setAttribute("type", "hidden");
		    objValIdEle.setAttribute("name", "db_id_deleted" + this.deletedIndex);
		    objValIdEle.setAttribute("id", "db_id_deleted" + this.deletedIndex);
	    	objValIdEle.setAttribute("value", db_id);
            var oNodeToRemove = Dom.get(this.id + 'objectiveValueRow' + index);
            oNodeToRemove.parentNode.appendChild(objValIdEle);
            document.getElementById('objectiveValueDeletedIndex').setAttribute("value", this.deletedIndex);
            oNodeToRemove.parentNode.removeChild(oNodeToRemove);

            if (this.numberObjectiveValues != removedIndex) {
               for(var x = removedIndex + 1; x < this.numberObjectiveValues; x++) {
                   Dom.get(this.id + 'objectiveValue' + x).setAttribute("name", this.id +"objectiveValue" + (x-1));
                   Dom.get(this.id + 'objectiveValue' + x).setAttribute("id", this.id +"objectiveValue" + (x-1));
                   Dom.get(this.id + 'assigned_user_id' + x).setAttribute("name", this.id +"assigned_user_id" + (x-1));
                   Dom.get(this.id + 'assigned_user_id' + x).setAttribute("id", this.id +"assigned_user_id" + (x-1));
                   Dom.get('db_id' + x).setAttribute("name", "db_id" + (x-1));
                   Dom.get('db_id' + x).setAttribute("id", "db_id" + (x-1));
                   
	       		   removeFromValidate('EditView', this.id + "assigned_user_id" + x);
	    		   removeFromValidate('EditView', this.id + "objectiveValue" + x);
	   	    	   addToValidate('EditView', this.id + "assigned_user_id" + (x-1), 'duplicate', false, SUGAR.language.get('OBJ_Objectives', 'ERROR_DUPLICATE_USER'));
	   	    	   addToValidate('EditView', this.id + "objectiveValue" + (x-1), 'float', false, SUGAR.language.get('OBJ_Objectives', 'ERROR_NOT_A_NUMBER'));		    	

                   var rButton = Dom.get(this.id + 'removeButton' + x);
                   rButton.setAttribute("name", (x-1));
                   rButton.setAttribute("id", this.id + "removeButton" + (x-1));
                   Dom.get(this.id + 'objectiveValueRow' + x).setAttribute("id", this.id + 'objectiveValueRow' + (x-1));
               }
            }
			...
			// Update Objective Value List Size
		    var listSize = document.getElementById("objectiveValueListSize");
		    listSize.setAttribute("value", listSize.value - 1);

            // CL Fix for 17651
            if(this.numberObjectiveValues == 0) {
               return;
            }
		...
		forceSubmit : function () {
		    var theForm = Dom.get(this.objectiveValueView);
		    if(theForm) {
		       theForm.action.value = 'Save';
		       
		       if(!check_form(this.objectiveValueView)) {
		          return false;
		       }
		       
		       if(this.objectiveValueView == 'EditView') {
		          theForm.submit();
		       } else if(this.objectiveValueView == 'QuickCreate') {
		          SUGAR.subpanelUtils.inlineSave(theForm.id, theForm.module.value.toLowerCase());
		       }
		    } 
		} //forceSubmit
	};
	objectiveValueWidgetLoaded = true;
})();
					...
					if(typeof form[validate[formname][i][nameIndex]]  != 'undefined'){
						...
						if(!bail){
						    ...
						    case 'error':
								...
								break;
						    case 'duplicate':
						    	var key = "OBJ_Objectives0assigned_user_id";
						    	var input_name = validate[formname][i][nameIndex];
								var user_id = document.getElementById(input_name).value;
								var listSize = parseInt(document.getElementById('objectiveValueListSize').value);
								var isDuplicate = false;
								for (var lineIndex = 0; lineIndex < listSize; lineIndex++) {
									if (input_name != key + lineIndex && user_id == document.getElementById(key + lineIndex).value) {
										isDuplicate = true;
										break;
									}
								}
								if (isDuplicate) {
							    	isError = true;
		                            add_error_style(formname, input_name, validate[formname][i][msgIndex]);
								}
						    	break;
							}
 ...
 jQuery(document).ready(function($){
	 jQuery("input[id='objective_value']").bind("keyup",function(){field_check("objective_value")});
 });
 
 var field_check=function(id){
	 var bind_id = "#"+id
	 var obj_value=jQuery(bind_id).val();
	 if(obj_value!=""){
		 var pattern=/^\d*$/;
		 var flag=pattern.exec(obj_value);
		 if(flag){
			 if(obj_value <= 0){
				 alert("The objective value must be larger than zero");
				 jQuery(bind_id).attr("value","");
			 }
		 }else{
			 alert("The objective value must be a positive number");
			 jQuery(bind_id).attr("value","");
		 }
	 }
 }
