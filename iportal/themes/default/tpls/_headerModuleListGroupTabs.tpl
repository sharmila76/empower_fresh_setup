...
<ul>
    ...
    <li class="noBorder">
        <span class="currentTabLeft">&nbsp;</span><span class="currentTab">
            <a href="{iportal_link module=$modules.modules.0 data=$modules.modules.0 link_only=1 extraparams=$extraparams}"
    ...
    <span id="moduleLink_{$smarty.foreach.moduleList.index}" {if ( ( $parentTab == $group || (!$parentTab && in_array($MODULE_TAB,$modules.modules)) ) && !$groupSelected ) || ($smarty.foreach.moduleList.index == 0 && $defaultFirst)}class="selected" {assign var="groupSelected" value=true}{/if}>
    	<ul>
	        ...
	        <li>
	        	...
	        	{iportal_link id=$moduleTabId module=$module data=$module}
	        </li>
	        {/foreach}
	        ...
	        <li class="subTabMore">
		        ...
		        {foreach from=$subMoreModules.$group.modules item=submodule}
					<li>
						<a href="{iportal_link module=$submodule link_only=1 extraparams=$extraparams}">{$submodule}
						</a>
					</li>
		        {/foreach}
		        </ul> 
