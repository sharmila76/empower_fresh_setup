...
function processListViewMulti($seed, $xTemplateSection, $html_varName) {
	...
	echo "<form name='MassUpdate' method='post' action='iportal.php'>";
...
function getUserVariable($localVarName, $varName) {
	...
	*/
			...
			$export_link = '<input class="button" type="button" value="'.$this->local_app_strings['LBL_EXPORT'].'" ' .
					'onclick="return sListView.send_form(true, \''.$_REQUEST['module'].'\', \'iportal.php?entryPoint=export\',\''.$this->local_app_strings['LBL_LISTVIEW_NO_SELECTED'].'\')">';
			...
			if($user_merge == 'on' && isset($admin->settings['system_mailmerge_on']) && $admin->settings['system_mailmerge_on']) {
				...
				function mailmerge_overlib() {
					return overlib('<a style=\'width: 150px\' class=\'menuItem\' onmouseover=\'hiliteItem(this,\"yes\");\' onmouseout=\'unhiliteItem(this);\' onclick=\'return sListView.send_form(true, \"MailMerge\", \"iportal.php\", \"{$this->local_app_strings['LBL_LISTVIEW_NO_SELECTED']}\")\' href=\'#\'>{$this->local_app_strings['LBL_LISTVIEW_OPTION_SELECTED']}</a>"
						. "<a style=\'width: 150px\' class=\'menuItem\' onmouseover=\'hiliteItem(this,\"yes\");\' onmouseout=\'unhiliteItem(this);\' href=\'iportal.php?action=index&module=MailMerge\'>{$this->local_app_strings['LBL_LISTVIEW_OPTION_CURRENT']}</a>"
						. "<a style=\'width: 150px\' class=\'menuItem\' onmouseover=\'hiliteItem(this,\"yes\");\' onmouseout=\'unhiliteItem(this);\' href=\'iportal.php?action=index&module=MailMerge&entire=true\'>{$this->local_app_strings['LBL_LISTVIEW_OPTION_ENTIRE']}</a>"
			...
			if($this->show_paging == true) {
				...
				if ($subpanel_def != null) {
	            	include_once('iportal/include/SubPanel/IportalSubPanelTiles.php');
	            	$subpanelTiles = new IportalSubPanelTiles($sugarbean);
	...
	{
		require_once('iportal/include/generic/LayoutManager.php');
