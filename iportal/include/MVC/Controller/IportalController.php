<?php
/**
 * TK - Probally this is the point that we need to take out everything
 * The original controller don't accept to be extended so basically we are building a custom one
 */
require_once('iportal/include/iportal_utils.php');
require_once('iportal/include/MVC/View/IportalView.php');
require_once('include/MVC/Controller/SugarController.php');

class IportalController {
	...
	private function loadMapping($var, $merge = false){
		...
		if(!$$var){
			...
			if(file_exists('iportal/include/MVC/Controller/'.$var.'.php')) {
				require('iportal/include/MVC/Controller/'.$var.'.php');
			}
			if(file_exists('iportal/modules/'. $this->module .'/' . $var . '.php')){		
				require('iportal/modules/'. $this->module .'/' . $var . '.php');
			}
	...
	private function processView(){
		$view = IportalViewFactory::loadView($this->view, $this->module, $this->bean, $this->view_object_map);
	...
	protected function post_save(){
		...
		$url = "iportal.php?module=".$module."&action=".$action."&record=".$id;
	...
	protected function post_delete(){
		...
		$return_id = isset($_REQUEST['return_id']) ?
			$_REQUEST['return_id'] :
			'';
		$url = "iportal.php?module=".$return_module."&action=".$return_action."&record=".$return_id;
		$this->set_redirect($url);
	}
	/**
	...
	protected function post_massupdate(){
		...
		$return_action = isset($_REQUEST['return_action']) ? 
			$_REQUEST['return_action'] : 
			$GLOBALS['sugar_config']['default_action'];
		$url = "iportal.php?module=".$return_module."&action=".$return_action;
