...
require_once('iportal/include/SubPanel/IportalSubPanelTilesTabs.php');
require_once('iportal/include/SubPanel/IportalSubPanelDefinitions.php');

class IportalSubPanelTiles extends SubPanelTiles
{
	function IportalSubPanelTiles(&$focus, $layout_def_key='', $layout_def_override = '')
	{
		$this->focus = $focus;
		$this->id = $focus->id;
		$this->module = $focus->module_dir;
		$this->layout_def_key = $layout_def_key;
		$this->subpanel_definitions=new IportalSubPanelDefinitions($focus, $layout_def_key, $layout_def_override);
	}
    ...
    function getSelectedGroup()
    {
		...
		//TP-NXT #1346 - TK - Fix for incorrect theme on save and tab group
	...
	function getTabs($showTabs = true, $selectedGroup='')
    {
        ...
        {
	        return IportalSubPanelTilesTabs::getTabs($tabs, $showTabs, $selectedGroup);
	    }
        else
        {
	...
	function display($showContainer = true, $forceTabless = false) 
{
		...
		ob_start();
    echo '<script type="text/javascript" src="'. getJSPath('iportal/include/SubPanel/SubPanelTiles.js') . '"></script>';
?>
<script>
...
<?php
        ...
        foreach ($tabs as $tab)
		{
			...
			if (empty($this->show_tabs))
			{			
				//TP-NXT #1346 - TK - Fix for incorrect theme on hiding and showing supanels
...
EOQ;
            ...
            if($div_display != 'none'){
            	...
            	ob_start();
            	include_once('iportal/include/SubPanel/IportalSubPanel.php');
            	$subpanel_object = new IportalSubPanel($this->module, $_REQUEST['record'], $tab,$thisPanel,$layout_def_key);
            	$subpanel_object->setTemplateFile('iportal/include/SubPanel/SubPanelDynamic.html');
