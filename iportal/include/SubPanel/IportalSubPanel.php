...
class IportalSubPanel
{
    ...
    var $layout_def_key='';
	function IportalSubPanel($module, $record_id, $subpanel_id, $subpanelDef, $layout_def_key='')
	{
		...
		if (empty($subpanelDef)) {
			...
			$panelsdef=new IportalSubPanelDefinitions($result,$layout_def_key);
	...
	function ProcessSubPanelListView($xTemplatePath, &$mod_strings)
	{
		...
		{	
			...
			$header_text= "&nbsp;<a href='iportal.php?action=index&module=DynamicLayout&from_action=$file_name&from_module=$mod_name&mod_lang="
  ...
  function getModuleSubpanels($module){
  	require_once('iportal/include/SubPanel/IportalSubPanelDefinitions.php');
  		...
  		$mod = new $class();
  		$spd = new IportalSubPanelDefinitions($mod);
...
//  	$GLOBALS['log']->debug('SubPanel.php->saveSubPanelDefOverride(): '.$name);
	...
	{
		$default_subpanel_define = IportalSubPanel::_get_default_subpanel_define($module, $subpanel_id);
		$custom_subpanel_define = IportalSubPanel::_get_custom_subpanel_define($module, $subpanel_id);
