...
class Popup_Picker
{
	...
	function Popup_Picker()
	{
		...
		if(empty($popupMeta)){
			...
			else if(file_exists('iportal/modules/'.$currentModule.'/metadata/popupdefs.php'))
				require_once('iportal/modules/'.$currentModule.'/metadata/popupdefs.php');
			else
		...
		if(isset($this->_popupMeta['create']['formBase']) && isset($_REQUEST['create']) && $_REQUEST['create'] == 'true') { // include create form
            //taras include the iportal formbase.php
              ...
              //end taras
            else require_once('modules/' . $currentModule . '/' . $this->_popupMeta['create']['formBase']);
			$this->_create = true;
		}
	}
	...
	function process_page()
	{
		...
		$output_html = "<script type=\"text/javascript\" src=\"" . getJSPath('iportal/include/javascript/iportal_3.js'). "\"></script>";
		$where = '';
...
EOQ;
		...
		else if (file_exists('iportal/modules/'.$currentModule.'/Popup_picker.html')){
			$form = new XTemplate('iportal/modules/'.$currentModule.'/Popup_picker.html');
		} 
		else {
