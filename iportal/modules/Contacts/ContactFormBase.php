...
function checkForDuplicates($prefix){
	global $local_log;
    require_once('iportal/include/formbase.php');
...
function buildTableForm($rows, $mod=''){
	...
	if ($action != 'ShowDuplicates')
	{
		...
		$form .= "<form action='iportal.php' method='post' name='dupContacts'>
		...
		foreach ($row as $key=>$value){
				if($key != 'id'){
					if(isset($_POST['popup']) && $_POST['popup']==true){
						$form .= "<td scope='row'><a  href='#' onclick=\"window.opener.location='iportal.php?module=Contacts&action=DetailView&record=${row['id']}'\">$value</a></td>\n";
					}
					else if(!$wasSet){
						$form .= "<td scope='row'><a target='_blank' href='iportal.php?module=Contacts&action=DetailView&record=${row['id']}'>$value</a></td>\n";
						$wasSet = true;
					}else{
					    $form .= "<td><a target='_blank' href='iportal.php?module=Contacts&action=DetailView&record=${row['id']}'>$value</a></td>\n";
					}
				}
		}
...
$the_form .= <<<EOQ
		...
		<form name="${prefix}ContactSave" onSubmit="return check_form('${prefix}ContactSave')" method="POST" action="iportal.php">
...
function handleSave($prefix, $redirect=true, $useRequired=false){
	...
	if (empty($_POST['record']) && empty($_POST['dup_checked'])) {
		...
		if(isset($duplicateContacts)){
            ...
            if (!empty($_POST['is_ajax_call']) && $_POST['is_ajax_call'] == '1')
            {
                ...
                $_SESSION['SHOW_DUPLICATES'] = $get;
                header("Location: iportal.php?$location");
            }
            return null;
		}
	}
	...
	if(isset($_REQUEST['inbound_email_id']) && !empty($_REQUEST['inbound_email_id'])) {
		...
		header("Location: iportal.php?&module=Emails&action=EditView&type=out&inbound_email_id=".$_REQUEST['inbound_email_id']."&parent_id=".$email->parent_id."&parent_type=".$email->parent_type.'&start='.$_REQUEST['start'].'&assigned_user_id='.$current_user->id);
		exit();
	}
	...
	if(isset($_POST['popup']) && $_POST['popup'] == 'true') {
		...
		$get .= '&query=true';
		header("Location: iportal.php?$get");
		return;
	}
...
function handleRedirect($return_id){
	...
	header("Location: iportal.php?action=$return_action&module=$return_module&record=$return_id");
}
