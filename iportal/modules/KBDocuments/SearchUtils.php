 ...
 ********************************************************************************/
	...
	require_once('iportal/modules/KBDocuments/metadata/KBSearchlistviewdefs.php');
    require_once('iportal/modules/KBDocuments/KBListViewData.php');
    ...
    if(is_string($qry_arr)){
        ...
        if(isset($qry_arr['custom_from'])){
            $params['custom_from']=" inner join kbdocuments_cstm cs on  kbdocuments.id=cs.id_c and cs.show_in_portal_c=1 ";
    ...
    $lv->setup($seedDocument, 'iportal/modules/KBDocuments/SearchListView.tpl', $where, $params);
...
function get_admin_fts_list($where,$isMultiSelect=false){
    ...
    $lv->delete = false;
    $lv->select = false;
    $lv->setup($seedCase, 'iportal/modules/KBDocuments/AdminSearchListView.tpl', $where, $params);
	$lv->show_mass_update_form=false;
	$savedSearchName = empty($_REQUEST['saved_search_select_name']) ? '' : (' - ' . $_REQUEST['saved_search_select_name']);
