...
function checkForDuplicates($prefix){
	if (!function_exists('checkRequired')) require_once('iportal/include/formbase.php');
...
function buildTableForm($rows, $mod='Accounts'){
	...
	{
		$form = "<form action='iportal.php' method='post' id='dupAccounts'  name='dupAccounts'><input type='hidden' name='selectedAccount' value=''>";
		...
		foreach ($row as $key=>$value){
				if($key != 'id'){
                    if(isset($_POST['popup']) && $_POST['popup']==true){
                        $form .= "<td scope='row'><a  href='#' onclick=\"window.opener.location='iportal.php?module=Accounts&action=DetailView&record=${row['id']}'\">$value</a></td>\n";
                    }   
                    else
					    $form .= "<td><a target='_blank' href='iportal.php?module=Accounts&action=DetailView&record=${row['id']}'>$value</a></td>\n";
...
$the_form .= <<<EOQ
		<form name="${prefix}AccountSave" onSubmit="return check_form('${prefix}AccountSave');" method="POST" action="iportal.php">
