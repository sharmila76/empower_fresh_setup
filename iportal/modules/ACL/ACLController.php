...
require_once('iportal/modules/ACL/IportalACLAction.php');
...
class ACLController {
	...
	function checkAccess($category, $action, $is_owner=false, $type='module'){
		...
		if($category == 'Calendar'){
			return IportalACLAction::userHasAccess($current_user->id, 'Calls', $action,$type, $is_owner) || IportalACLAction::userHasAccess($current_user->id, 'Meetings', $action,'module', $is_owner) || IportalACLAction::userHasAccess($current_user->id, 'Tasks', $action,'module', $is_owner);
		}
		if($category == 'Activities'){
			return IportalACLAction::userHasAccess($current_user->id, 'Calls', $action,$type, $is_owner) || IportalACLAction::userHasAccess($current_user->id, 'Meetings', $action,'module', $is_owner) || IportalACLAction::userHasAccess($current_user->id, 'Tasks', $action,'module', $is_owner)|| IportalACLAction::userHasAccess($current_user->id, 'Emails', $action,'module', $is_owner)|| IportalACLAction::userHasAccess($current_user->id, 'Notes', $action,'module', $is_owner);
		}
		return IportalACLAction::userHasAccess($current_user->id, $category, $action,$type, $is_owner);
	}
	
	function requireOwner($category, $value, $type='module'){
			global $current_user;
			if(is_admin($current_user))return false;
			return IportalACLAction::userNeedsOwnership($current_user->id, $category, $value,$type);
	}
	
	function filterModuleList(&$moduleList, $by_value=true){
		
		global $aclModuleList, $current_user;
		$actions = IportalACLAction::getUserActions($current_user->id, false);
