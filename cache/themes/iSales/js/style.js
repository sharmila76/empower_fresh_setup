...
YAHOO.util.Event.onAvailable('sitemapLinkSpan', function()
{
    document.getElementById('sitemapLinkSpan').onclick = function()
    {
        ...
        var smMarkup = '';
        var callback = {success: function(r) {
...
SUGAR.append(SUGAR.themes, {allMenuBars: {}, setModuleTabs: function(html) {
        ...
        for (var i = 0; i < nodes.length; i++) {
            currMenuBar = SUGAR.themes.currMenuBar = new YAHOO.widget.MenuBar(nodes[i].id, {autosubmenudisplay: true, visible: false, hidedelay: 750, lazyload: true});
            currMenuBar.render();
...
function toggleMoreTab() {
    $("#moreTab").toggle();
    var tabname = $("#more_to_hide").text();
    if (tabname == "Show More Tabs") {
        $("#more_to_hide").text("Hide tabs");
    }
    if (tabname == "Hide tabs") {
        $("#more_to_hide").text("Show More Tabs");
    }
}

//Added by Sharmila.
//On the browser, if the user clicks F1, show the details of the corresponding module.
//TODO: working only in the chrome. Should work in all browsers.
$(document).keydown(function(e) {
    if (e.keyCode == 112) {
        //Add module description here.
        var module_description = {"Accounts": "accounts module",
        ...
        var module_name = $.urlParam('module');
        event.preventDefault();
        alert(module_description[module_name]);
    }
});
